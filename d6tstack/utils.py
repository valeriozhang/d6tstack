...
def pd_to_mssql(df, uri, table_name, schema_name=None, if_exists='fail', tmpfile='mysql.csv', remotefile='mysql.csv'):
    ...
    if remotefile == 'mysql.csv':
        logger.send_log('creating ' + tmpfile, 'ok')
        df.to_csv(tmpfile, na_rep='', index=False)
        logger.send_log('loading ' + tmpfile, 'ok')
        if schema_name is not None:
            table_name = '{}.{}'.format(schema_name,table_name)
        sql_load = "BULK INSERT {} FROM '{}'; COMMIT;".format(table_name, tmpfile)
    else: 
        logger.send_log('creating ' + tmpfile, 'ok')
        df.to_csv(tmpfile, na_rep='', index=False, sep="|")
        logger.send_log('loading ' + tmpfile, 'ok')
        if schema_name is not None:
            table_name = '{}.{}'.format(schema_name,table_name)
        sql_load = "BULK INSERT {} FROM '{}' WITH ( FIRSTROW = 2, FIELDTERMINATOR = '|', ROWTERMINATOR = '0x0a' ); COMMIT;".format(table_name, remotefile)
        print( sql_load)
    sql_engine.execute(sql_load)
